...
const mailStringSha1 = `DKIM-Signature: v=1; a=rsa-sha1; s=brisbane; d=example.com;
      c=simple/simple; q=dns/txt; i=joe@football.example.com;
      h=Received : From : To : Subject : Date : Message-ID;
      bh=2jUSOH9NhtVGCQWNr9BrIAPreKQjO6Sn7XIkfJVOzv8=;
      b=AuUoFEfDxTDkHlLXSZEpZj79LICEps6eda7W3deTVFOk4yAUoqOB
...
func TestVerifyWithOption(t *testing.T) {
	...
	r = newMailStringReader(mailStringSha1)
	option = VerifyOptions{LookupTXT: net.LookupTXT, AllowSHA1: true}
	verifications, err = VerifyWithOptions(r, &option)
	if err != nil {
		t.Fatalf("Expected no error while verifying signature, got: %v", err)
	} else if len(verifications) != 1 {
	...
	v = verifications[0]
	if v.Err != nil && strings.Contains(v.Err.Error(), "hash algorithm too weak") {
		t.Fatalf("Expected verification to use sha1")
	}

	r = newMailStringReader(mailStringSha1)
	option = VerifyOptions{LookupTXT: net.LookupTXT, AllowSHA1: false}
	verifications, err = VerifyWithOptions(r, &option)
	if err != nil {
		t.Fatalf("Expected no error while verifying signature, got: %v", err)
	} else if len(verifications) != 1 {
	...
	v = verifications[0]
	if v.Err != nil && strings.Contains(v.Err.Error(), "body hash") {
		t.Fatalf("Expected verification to not use sha1")
	}
}
